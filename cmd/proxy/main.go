...
//proxy/main.go: 这个程序是一个代理服务器，它的主要功能是处理从远程存储下载和上传 Etcd 备份的请求。
//它实现了 Upload 和 Download 两个 gRPC 方法，分别用于上传和下载备份。
//backup-agent 和restore-agent子目录下面的main.go分别调用了Upload 和 Download 两个 gRPC 方法
